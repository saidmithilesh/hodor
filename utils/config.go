...
func (conf *Config) scan(configFilePath string) []byte {
	filecontent, err := ReadContentFromFile(configFilePath)
	if err != nil {
...
func (gc *GatewayConfig) validateTLS(c *Config) {
	...
	gc.TLSCertFilePath = GetFullPath(gc.TLSCertFilePath)
	gc.TLSKeyFilePath = GetFullPath(gc.TLSKeyFilePath)

	if !IsValidPath(gc.TLSCertFilePath) {
		log.Printf("\t - Error.InvalidCertPath :: Invalid filepath '%s' provided for TLS certificate file\n", gc.TLSCertFilePath)
		c.ValidationFailed = true
	}

	if !IsValidPath(gc.TLSKeyFilePath) {
...
func ResolveConfigFilePath() string {
	...
	absConfigPath := GetFullPath(*configFilePath)

	// Raise a Fatal error if the filepath is not valid
	if !IsValidPath(absConfigPath) {
		log.Fatalf(
